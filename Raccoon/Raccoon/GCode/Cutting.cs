...
namespace Raccoon.GCode
{
 ...
 string filename = "P1234567", double Zsec = 650, double Speed = 20000, double RetreatDistance = 70, double angles = 80)
        {
            List<string> ncc = new List<string> {
                "G"+Raccoon_Library.GlobalParameters.zero_point.ToString() + " (G54/G55/G56_zero_in_maka)",
                ...
                "S" + Raccoon_Library.Utilities.GeometryProcessing.Lerp(tool.prescribedSpindleSpeed, tool.maxSpindleSpeed, 1.0).ToString() + " M3" + " (sxx_-_speed,_m3_-_clockwise)", //slow rmp "S2500 M3 M1"
                "G0 G49 Z"+Raccoon_Library.GlobalParameters.z_safety.ToString() + " (go_to_safe_z_position_to_avoid_collision)" ,
            ...
            if (GlobalParameters.skirt_down)
                ncc.Insert(ncc.Count - 3, "P4010:-310 (skirt down)");
            else
                ncc.Insert(ncc.Count - 3, "P4010:0 (skirt up)");
            ...
            for (int i = 0; i < polylines.Count; i++)
            {
                ...
                // to avoid collision between the motor while travelling from polyline to polyline
                if (GlobalParameters.orient_to_zero_after_each_cut)
                    ncc.Add("G1 F20000 " + Axes.A + "0 " + Axes.B + "0 " + " (travel safely without hit of motor)");
...
double toolr = 10, double Zsec = 650, double Speed = 20000, double RetreatDistance = 70, int infeed = 2, bool Notch = true, bool pairing = false, double angleTol = 60, double maxAngle = 80, string filename = "P1234567")
        {
            List<string> ncc = new List<string> {
                "G"+Raccoon_Library.GlobalParameters.zero_point.ToString() + " (G54/G55/G56_zero_in_maka)",
                //"P4010:250 (lift_aspiration,_plastic_cover_for_ventilation)", //lift aspiration
            ...
            List<List<Curve>> pairs = Raccoon_Library.Utilities.GeometryProcessing.FindPairs(crvs, pairing);
            ...
            for (int p = 0; p != pairs.Count; p++)
            {
                ...
                if (angleTol > 0)
                {
                    var check = Raccoon_Library.Utilities.GeometryProcessing.CheckAngle(ply0, ply1, angleTol);
                    ply0 = check.Item1;
                    ply1 = check.Item2;
                ...
                string orient = (Raccoon_Library.Utilities.GeometryProcessing.IsClockwiseClosedPolylineOnXYPlane(ply0)) ? "CounterClockwise" : "Clockwise";
                ...
                if (ply0.IsValid && ply1.IsValid)
                {
                    ...
                    for (int k = infeed; k >= 1; k--)                           // iterate infeeds
                    {
                        ...
                        for (int i = 0; i != vrts.Length - 1; i++)              // iterate segments
                        {
                            ...
                            List<Point3d> ptsl = Raccoon_Library.Utilities.GeometryProcessing.DividePoints(pts[0], pts[1], IPDtemp);
                            List<Point3d> ptsm = Raccoon_Library.Utilities.GeometryProcessing.DividePoints(p30, p21, IPDtemp);
                            List<Point3d> ptsu = Raccoon_Library.Utilities.GeometryProcessing.DividePoints(pts[3], pts[2], IPDtemp);
                            ...
                            for (int m = 0; m != ptsl.Count; m++)
                            {
                                ...
                                TCP2 = Raccoon_Library.Utilities.GeometryProcessing.VecPlnInt(TCP, n, RetreatDistance);              //#retreat pt
                                TCP3 = Raccoon_Library.Utilities.GeometryProcessing.VecPlnInt(TCP, n, Zsec);             //#safe pt
   ...
   string filename = "P1234567", double Zsec = 650, double Speed = 20000, double RetreatDistance = 70, int infeed = 1)
        {
            List<string> ncc = new List<string> {
                "G"+Raccoon_Library.GlobalParameters.zero_point.ToString() + " (G54 - default Maka/G55/G59_zero_in_maka)",
                //"P4010:250 (lift_aspiration,_plastic_cover_for_ventilation)", //lift aspiration
                "T" + tool.id.ToString() + " M6" + " (txx_-_tool_name,_m6_-_change_tool)", //get Toool
                "S" + Raccoon_Library.Utilities.GeometryProcessing.Lerp(tool.prescribedSpindleSpeed, tool.maxSpindleSpeed,1.0).ToString() + " M3" + " (sxx_-_speed,_m3_-_clockwise)", //slow rmp "S2500 M3 M1"
                //"G47" +Axes.DefaultRotation +" F5000" + " (g47_-_3_axis_in_plane)",//neutral position "G1 G47 A0 B0 F5000 M1"
            ...
            foreach (Line l in lines)
            {
                ...
                for (int j = 0; j < multiLines.Count; j++)
                {
                    ...
                    ncc.Add(GCode.CoordinateSystem.Pt2nc(sp, 3, "")); // travel without rotation
                    ncc.Add(AB.Item3);  //rotate in
                ...
                // set all rotations to zero to avoid motor hitting the timber
                if (GlobalParameters.orient_to_zero_after_each_cut)
                    ncc.Add("G1 F20000 " + Axes.A + "0 " + Axes.B + "0 " + " (travel safely without hit of motor)");
            }
