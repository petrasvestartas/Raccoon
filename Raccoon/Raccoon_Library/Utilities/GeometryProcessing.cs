...
namespace Raccoon_Library.Utilities
{
    public static class GeometryProcessing
    {
        ...
        public static Mesh MeshPipe(Plane plane, int sides = 10, double height = 1, double radius = 1)
        {
            Mesh mesh = new Mesh();

            Point3f c0 = new Point3f(0, 0, 0);
            Point3f c1 = new Point3f(0, 0, 1);
            ...
            {
                float heading = a * (float)Math.PI / 180;
                Point3f p = new Point3f((float)Math.Cos(heading) * 1, (float)Math.Sin(heading) * 1, 0);
                mesh.Vertices.Add(p);
            }
            ...
            {
                float heading = a * (float)Math.PI / 180;
                Point3f p = new Point3f((float)Math.Cos(heading) * 1, (float)Math.Sin(heading) * 1, 1);
                mesh.Vertices.Add(p);
            }
            ...
            for (int i = 0; i < n - 1; i++)
            {
                mesh.Faces.AddFace(i + 2, i + 1 + 2, i + n + 1 + 2, i + n + 2);
            }
            mesh.Faces.AddFace(n - 1 + 2, 0 + 2, n + 2, n * 2 - 1 + 2);

            for (int i = 0; i < n - 1; i++)
            {
                mesh.Faces.AddFace(0, i + 2 + 1, i + 2);
                mesh.Faces.AddFace(i + 2 + n, i + 2 + 1 + n, 1);
            }

            mesh.Faces.AddFace(2, 2 + n - 1, 0);
            mesh.Faces.AddFace(mesh.Vertices.Count - 1, 0 + 2 + n, 1);

            mesh.Transform(Transform.Scale(Plane.WorldXY, radius, radius, height));
        ...
        public static Polyline[] NotchSpiral(this Line L, double R, double S)
        {
            ...
            Polyline p0 = Polygon((int)(Math.Max(3, R * 1.00 / Math.Max(0.1, S))), (Math.Max(1, R)), plane, 0, false);
            ...
            Polyline[] p = InterpolatePolylines(p0, p1, (int)Math.Max(0, (L.Length / R) - 1));
        ...
        public static Mesh TransformMesh(this Mesh m, Plane plane, double radius, double height)
        {
