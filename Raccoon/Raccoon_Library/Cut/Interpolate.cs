...
namespace Raccoon_Library
{
    public static class Interpolate
    {
        public static Polyline[] InterpolateTwoPolylinesToOnePath(Polyline C0, Polyline C1, int Infeed, double R, bool soft, List<Line> DrillLines = null)
        {
            ...
            //Rhino.RhinoDoc.ActiveDoc.Objects.AddPointCloud(new PointCloud(interpolated[0]));
            return interpolated;
        }
        ...
        public static Polyline[] InterpolatePolylinesZigZag(Polyline path, Polyline normal, int n, bool soft)
        {
            ...
            //Create array of interpolated polylines
            Polyline[] interpolatedPolylines = new Polyline[n + 2];

            for (int j = 0; j < n + 2; j++)
                interpolatedPolylines[j] = new Polyline();

            //Add interpolated points to each polyline
            ...
            //Rhino.RhinoDoc.ActiveDoc.Objects.AddPointCloud(new PointCloud(normal));
            ...
            //Rhino.RhinoDoc.ActiveDoc.Objects.AddPointCloud(new PointCloud(ZigZag));
