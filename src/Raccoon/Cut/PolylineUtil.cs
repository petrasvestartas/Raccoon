...
using System.Linq;
using Raccoon.Clipper;
using Raccoon.Utilities;
using Rhino;
    ...
    public static class PolylineUtil {
        ...
        public static Tuple<SortedDictionary<int, Polyline>, SortedDictionary<int, Polyline>> SplitCurvesByPlane(this Polyline pline, Plane plane)
        {
            ...
            foreach (Curve sp in splitCurves) {
                ...
                if (sp.TryGetPolyline(out Polyline cutPline))  {
                    ...
                    if (plane.IsAbovePlane(sp.PointAtNormalizedLength(0.5))>-1) { //if (cp0.DistanceToSquared(pt) < cp1.DistanceToSquared(pt)){ //Distance is positive or negative based on plane normal
      ...
      new Line((lA.To + lB.To) * 0.5, (lA.From + lB.From) * 0.5),
        ...
        public static bool IsPointInside(this Polyline polyline_, Point3d p_, double scale = 1e10, double tol = 0.001)
        {
            ...
            Transform xform = Rhino.Geometry.Transform.PlaneToPlane(plane, Plane.WorldXY);
            polyline.Transform(xform);
            var input = Raccoon.Offset.PolylineToIntPoint(polyline, scale);
            ...
            bool isInside = Raccoon.Clipper.Clipper.PointInPolygon(inputP, input) != 0;
        ...
        public static Polyline InsertClosedPlineToClosedPline(Polyline jointBBox, Polyline jointPolyline, Polyline platePolyline, Polyline platePolylineBackUp, bool takeOutside = false)
        {
            ...
            try {
                ...
                if (jointPolyline.Count == 5) {
                    ...
                    if (takeOutside) {
                        p0Joint = IsPointInside(y, pt0, 1e10, 0.0001) ? p0B : p0A;//0.0001
                    } else {
                        p0Joint = IsPointInside(y, pt0, 1e10, 0.0001) ? p0A : p0B;//0.0001
                    }
